...
class Solution {
  public:
    ...
    Node* getTree(int pre[],int size,int &i, int bound)
    {
        if(i==size || pre[i] > bound)
            return NULL;
        Node* node = newNode(pre[i]);
        i++;
        node->left = getTree(pre,size,i,node->data);
        node->right = getTree(pre,size,i,bound);
        return node;
    }
    Node* Bst(int pre[], int size) {
        int i = 0;
        Node* tree = getTree(pre,size,i,INT_MAX);
        return tree;
        // code here
    }
};
